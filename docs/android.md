...
长按桌面空白处 → 小组件（华为叫"服务卡片"或"窗口小工具"）→ **DSH 状态**，拖到桌面上。它显示：

- **几个等你确认**：DSH 的审批和提问，加上 Claude Code / Codex 的确认请求。有待确认时这一行变成橙色；
- **几个在跑**：正在运行的 DSH 会话（不含子代理）；
- 连接状态：已连接 / 未连接 / 需要登录；
- 最近完成（或出错）的任务和时间。

点一下打开 App。

小组件本身不联网、不定时刷新：数字由电脑上的插件在变化时通过后台提醒那条连接推过来（`st` 帧），后台服务收到后重画小组件，所以几乎不耗电。也因此，后台服务没在运行时（例如重启后没开"自启动"），小组件会停在最后的状态，并显示"未连接"。
