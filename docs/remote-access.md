...
1. **一条有认证的通道**，把手机的请求送到电脑上的 `127.0.0.1:3080`。认证可以是 VPN，也可以是带登录页的网关。
...
```bash
curl -H "Host: 100.64.0.5:8080" http://127.0.0.1:3080/m/api/ping
# {"ok":true,...}                           → 这个地址已被信任
...
## 方案 A：VPN + `tools/forward.mjs`（最简单）

适用于 Tailscale、WireGuard、ZeroTier 这类 VPN：手机和电脑处在同一个私有网络里。

1. 在电脑和手机上都装好 VPN，记下电脑的 VPN 地址，例如 `100.64.0.5`。
2. 在电脑上运行：
...
4. 在手机 App 里填 `100.64.0.5:8080`。私有地址会自动走 `http://`；VPN 本身已经加密，所以这样没问题。
...
- 要打开"失败后重启"：VPN 还没连上时监听会失败，`forward.mjs` 会以退出码 1 退出，等下一次重启再试。
...
作者自用的就是这种，适合不想在手机上常开 VPN 的情况。

```
手机 ──HTTPS──▶ 你的服务器：反向代理 + 登录 ──隧道（如 WireGuard）──▶ 家里电脑 ──▶ 127.0.0.1:3080
```

### 登录方式：必须是"登录页 + Cookie"

可以用表单登录，也可以用 oauth2-proxy、Authelia、Authentik 这类认证代理。
**HTTP Basic 认证不行**：App 里的 WebView 不会弹 Basic 认证对话框。
...
这样 DSH 和插件都会把请求当作本机请求，不需要配置 `trustedHosts`。作者自用的正是这种。

> 验证情况：
> - 做法 2 在作者的部署里长期使用。
> - 做法 1 所依赖的插件逻辑，有单元测试和本机转发端到端测试覆盖。
...
| 提醒完全不来 | 先看 [android.md](android.md) 里的电池设置 |
